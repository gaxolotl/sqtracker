...
// Non-globally-routable IPv4 ranges. Addresses that are not "private" in the
// RFC1918 sense but must still never be treated as publicly reachable when
// deciding whether it is safe to connect to a user-supplied address.
// Covers GHSA-2p57-rm9w-gvfp (isPublic() returned true for reserved ranges).
const ipv4NonPublicRanges = [
  ...
  { base: 0xc0000000, mask: 0xffffff00 }, // 192.0.0.0/24 IETF protocol assignments
  { base: 0xc0000200, mask: 0xffffff00 }, // 192.0.2.0/24 TEST-NET-1
  { base: 0xc0586300, mask: 0xffffff00 }, // 192.88.99.0/24 6to4 relay anycast
  ...
  { base: 0xffffffff, mask: 0xffffffff }, // limited broadcast
];
...
ip.isPublic = function (addr) {
  ...
  if (!/:/.test(addr)) {
    // IPv4 literal. (The upstream ipv6 regex also matches dotted quads, so a
    // colon check is used here instead of isV6Format.)
    ...
    for (let i = 0; i < ipv4NonPublicRanges.length; i++) {
      const { base, mask } = ipv4NonPublicRanges[i];
      if (((long & mask) >>> 0) === base) return false;
    }
  } else if (/^ff[0-9a-f]{2}:/i.test(addr)) {
    // IPv6 multicast is never a public unicast destination
    return false;
  }
