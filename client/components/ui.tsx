  ...
  return (
    <header className="page-header">
      <div className="page-title-row">
        <h1>{title}</h1>
        {info ? <span className="info-tooltip" tabIndex={0} aria-label={info}><CircleHelp aria-hidden="true" /><span role="tooltip">{info}</span></span> : null}
      </div>
...
export function ApiState({ loading, error, empty, children }: { loading?: boolean; error?: string; empty?: boolean; children: React.ReactNode }) {
  const { t } = useI18n();
  if (loading) return <div className="state-panel"><LoaderCircle className="spin" aria-hidden="true" /> {t("loading")}</div>;
  if (error) return <div className="state-panel state-error"><AlertCircle aria-hidden="true" /><span>{error}</span></div>;
...
export function SignInRequired() {
  ...
  return (
    <div className="state-panel sign-in-panel">
      ...
      <Link className="primary-button compact-button" href="/login">{t("login")}</Link>
    </div>
  );
}
