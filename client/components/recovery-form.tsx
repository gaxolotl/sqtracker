...
export function RecoveryForm({ stage }: { stage: "initiate" | "finalise" }) {
  ...
  return <main className="page form-page"><PageHeader title={stage === "initiate" ? "Reset password" : "Choose a new password"} /><form className="stack-form auth-card recovery-card" onSubmit={submit}><Field label="Email"><input type="email" name="email" required /></Field>{stage === "finalise" ? <Field label="New password"><input type="password" name="newPassword" autoComplete="new-password" required /></Field> : null}<ActionMessage message={message} error={error} /><button className="primary-button full-button" type="submit">{stage === "initiate" ? "Send reset link" : "Update password"}</button><p className="auth-switch"><Link href="/login">Back to login</Link></p></form></main>;
}
